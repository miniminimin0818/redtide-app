...
def assess_red_tide_risk(temp, salt):
    ...
    if 24 <= temp <= 27:
        risk_score += 50
        reasons.append("🌡️ **수온(24~27℃)**: 적조 생물 증식에 최적입니다.")
    elif temp >= 28:
        risk_score += 30
        reasons.append("🌡️ **고수온(28℃↑)**: 성장이 다소 둔화될 수 있으나 주의가 필요합니다.")
    else:
        reasons.append("❄️ **저수온**: 수온이 낮아 적조 발생 확률이 낮습니다.")

    # 염분 평가
    if 30 <= salt <= 34:
        risk_score += 50
        reasons.append("🧂 **염분(30~34psu)**: 적조 생물 증식에 최적입니다.")
    elif salt < 28:
        risk_score -= 20
        reasons.append("💧 **저염분**: 염분이 너무 낮아 적조 생물이 사멸할 가능성이 있습니다 (단, 패류 폐사 주의).")
    else:
    ...
    if risk_score >= 90:
        ...
        return "⚠️ 주의 (발생 가능 조건 충족)", "orange", reasons
    else:
        return "✅ 안전 (발생 확률 낮음)", "green", reasons
...
def main():
    ...
    with tab3:
        ...
        if st.checkbox("산점도 그래프 보기 (데이터가 많아 로딩에 시간이 걸릴 수 있습니다)", value=True):
